import React, { useEffect, useState } from 'react'

import Button from '@mui/material/Button'
import TextField from '@mui/material/TextField'
import Autocomplete from '@mui/material/Autocomplete'
import Card from './Card'

const SelectCard = (props) => {
  const [cardArray, setCardArray] = useState([])
  const [selectedTeam, setSelectedTeam] = useState(null)
  const [playerOption, setPlayerOption] = useState(props.dataCard)

  useEffect(() => {
    setPlayerOption(
      props.dataCard.filter((player) => {
        return !selectedTeam || selectedTeam === player.currentClub
      })
    )
  }, [selectedTeam, props.dataCard])

  const handleChangePlayer = (_, value) => {
    if (
      cardArray.filter((player) => player.fullName === value.fullName)
        .length < 1
    )
      setCardArray([...cardArray, value])
  }

  const handleChangeTeam = (_, value) => {
    setSelectedTeam(value ? value.teamName : null)
  }

   const resetButton = () => {
   setCardArray([])
  }

  return (
    <div
      className="selectPlayerContainer"
      style={{ display: 'flex', width: '80%' }}
    >
      <div
        style={{
          display: 'flex',
          alignItems: 'center',
          flexDirection: 'column',
        }}
      >
        <div style={{ fontSize: '1.2rem', fontWeight: '700', padding: '1rem' }}>
          Add a player
        </div>
        <Autocomplete
          onChange={handleChangePlayer}
          disablePortal
          id="combo-box-demo"
          options={playerOption}
          getOptionLabel={(option) => (option ? option.fullName : '')}
          sx={{ width: 300 }}
          renderInput={(params) => (
            <TextField {...params} label="Select Player" />
          )}
        />
        <div style={{ fontSize: '1.2rem', fontWeight: '700', padding: '1rem' }}>
          Select a team
        </div>
        <Autocomplete
          onChange={handleChangeTeam}
          disablePortal
          id="combo-box-demo"
          options={props.dataTeam}
          getOptionLabel={(option) => (option ? option?.teamName : '')}
          sx={{ width: 300 }}
          renderInput={(params) => (
            <TextField {...params} label="Select Team" />
          )}
        />
        <div style={{ marginTop: '1rem'}}>
          <Button
            onClick={resetButton}
            variant="contained"
          >
            Reset Players
          </Button>
        </div>
      </div>
      <div
        style={{
          display: 'flex',
          flex: '1',
          justifyContent: 'center',
          alignItems: 'center',
        }}
      >
        <Card dataCard={cardArray} />
      </div>
     
    </div>
  )
}

export default SelectCard
